...
void NsSkelRpcServer::serverThreadWork () {
	...
	while (serverActive.load ()) {
		...
		if (dataSocketFd < 0) {
			cout_lock.lock ();
			cerr << "accept() for the listening socket returned " << dataSocketFd << " (<0), errno=" << errno << "(" << (errno == EINTR ? "EINTR" : "not EINTR") << ")" << endl;
			cout_lock.lock ();
			continue;
		}

		_threads[dataSocketFd] = make_shared<thread> (std::bind (&NsSkelRpcServer::processIncomingConnection, this, dataSocketFd));
...
void NsSkelRpcServer::processIncomingConnection (int dataSocketFd) {
	...
	if (getpeername (dataSocketFd, (sockaddr*) & sa, &sl)) { // Need to call getpeername(), since accept() fails to supply valid data
		cout_lock.lock ();
	...
	if (getpeername (dataSocketFd, (sockaddr*) & sa, &sl)) { // Need to call getpeername() twice, because it sometimes fails to supply valid data on the first call
		cout_lock.lock ();
	...
	if (msglenrcount < ((ssize_t) sizeof (msglen))) {
		cout_lock.lock ();
		cerr << "processIncomingConnection(): Cannot read incoming message length (" << sizeof (msglen) << " bytes)" << endl;
		cout_lock.unlock ();
